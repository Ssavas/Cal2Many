   ...
   public int tmp=0;
	...
	public Object visit(Assign n, Object env) {
		if(n.e instanceof FunctionCall){
			FunctionCall fn = (FunctionCall)n.e;
			if(fn.i.s.startsWith("Mop_")){
				if(sBackend.equals("DAG")){
					if(fn.exs.size()==3)
						n.e = new MatrixOps(fn.i.s,fn.exs.get(0),null,fn.exs.get(1),fn.exs.get(2));
					else if(fn.exs.size()==4)
						n.e = new MatrixOps(fn.i.s,fn.exs.get(0), fn.exs.get(1),fn.exs.get(2),fn.exs.get(3));
				}
				else{
					fn.exs.add(0, new IdentifierExp(n.i.s));
					return new StatementCall(fn.i,fn.exs);
				}
...
/*		StatementList sts = new StatementList();
	...
	public Object visit(ListAssign n, Object env) {
		...
		if(n.es.size()==0 && n.len!=null){
	    	...
	    	sts.add(new VarDeclSimp(new IntegerType(32),new Identifier(ivar),new IntegerLiteral(0),true));
			sts.add(new For(ie,new IntegerLiteral(0),
					new LessOrEqual(ie,n.len.get(0)),
					new Plus(ie,new IntegerLiteral(1)),
					new ListAssign(new  ExpIndexer(n.i,ie),null,new ExpIndexer(n.e,ie),null, n.isListCopy,n.varT)
					));    
		   ...
		   if(n.colExp instanceof ListComprehension){
			   ...
			   sts.addAll((StatementList)n.colExp.accept(this, svar));			   
				...
				afor = new For(ie,
					    ...
					    new Plus(ie,new IntegerLiteral(1)),
						new Assign(new Identifier(((VarDeclSimp)n.vars.get(0)).i.s),new ExpIndexer(new IdentifierExp(svar),ie), null)
                        );
				return new Pair<StatementList, For>(sts,afor);			   
		   }
		}
		...
		if(n.eles.size()==1){
		 ...
		 ArrayList<Pair<StatementList, For>> sts_fors = new ArrayList<Pair<StatementList, For>>();
		   ...
		   for(Pair<StatementList, For> sts_for : sts_fors){
			...
			sts1.add(new ListAssign(new  ExpIndexer(new IdentifierExp(var),ie),null, (Exp)n.eles.get(0),null,false, null));

			sts1.add(new Assign(new Identifier(ivar),new Plus(ie,new IntegerLiteral(1)), null));

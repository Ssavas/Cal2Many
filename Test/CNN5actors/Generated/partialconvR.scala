...
class applyFilter() extends Module {
	val io = IO(new Bundle{
		...
		val convR_applyResult_out = Output(UInt(width = 32.W))
		val en = Input(Bool())
		val valid = Output(Bool())
	})

	//Define all versions of the variables
	val convR_applyResult_v1 = Wire(UInt(width = 32.W))
	...
	convR_applyResult_v1 := conv7_ret_v0	// convR_applyResult_v1 total delay: 17
	

	//Write the outputs
	// convR_applyResult_v1 total delay : 17
	io.convR_applyResult_out := convR_applyResult_v1

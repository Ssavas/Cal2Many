...
volatile int* beamer_2_ch2_ready SECTION(".core7.data");
volatile int* beamer_2_ch2_valid SECTION(".core7.data");
volatile int* beamer_2_ch3_ready SECTION(".core7.data");
volatile int* beamer_2_ch3_valid SECTION(".core7.data");

//OutputPorts
uint64_t* beamer_2_ch14_buffer SECTION(".core7.data");
uint64_t* beamer_2_ch15_buffer SECTION(".core7.data");
volatile int* beamer_2_ch14_ready SECTION(".core7.data");
volatile int* beamer_2_ch14_valid SECTION(".core7.data");
volatile int* beamer_2_ch15_ready SECTION(".core7.data");
volatile int* beamer_2_ch15_valid SECTION(".core7.data");
...
int32_t beamer_2_ra_val_counter SECTION(".core7.data") = 0;
int32_t beamer_2_rb_val_counter SECTION(".core7.data") = 0;
...
int beamer_2_checkFlag(volatile int* flag){
	while(*flag == 0);
	return 1;
}
...
static inline void beamer_2_read_ra_val ( ) {
	int64_t ra_val;
	while(*beamer_2_ch2_valid == 0);//Blocking read
	ra_val = *beamer_2_ch2_buffer; //read input 
	*beamer_2_ch2_ready = 1; // channel ready to receive new data
	*beamer_2_ch2_valid = 0; // no valid input left on the channel
	
	beamer_2_complex_ra[beamer_2_ra_val_counter][1] = ra_val;
	beamer_2_ra_val_counter = (beamer_2_ra_val_counter + 1);

}
static inline void beamer_2_read_ra_val_done ( ) {
	beamer_2_ra_val_counter = 0;
...
static inline void beamer_2_read_rb_val ( ) {
	int64_t rb_val;
	while(*beamer_2_ch3_valid == 0);//Blocking read
	rb_val = *beamer_2_ch3_buffer; //read input 
	*beamer_2_ch3_ready = 1; // channel ready to receive new data
	*beamer_2_ch3_valid = 0; // no valid input left on the channel
	
	beamer_2_complex_rb[beamer_2_rb_val_counter][1] = rb_val;
	beamer_2_rb_val_counter = (beamer_2_rb_val_counter + 1);

}
static inline void beamer_2_read_rb_val_done ( ) {
	beamer_2_rb_val_counter = 0;
...
static inline void beamer_2_correlation ( ) {
	...
	while(*beamer_2_ch14_ready == 0);// Wait for the ready signal. 
	...
	*beamer_2_ch14_valid = 1; //(because) channel has new data 
	...
	while(*beamer_2_ch15_ready == 0);// Wait for the ready signal. 
	for(int beamer_2_correlation_i = 0; beamer_2_correlation_i < 9; beamer_2_correlation_i++){
...
int int_state_vars_beamer_2 SECTION(".core7.data") = 1;

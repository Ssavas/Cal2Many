...
volatile int* beamer_3_ch4_ready SECTION(".core8.data");
volatile int* beamer_3_ch4_valid SECTION(".core8.data");
volatile int* beamer_3_ch5_ready SECTION(".core8.data");
volatile int* beamer_3_ch5_valid SECTION(".core8.data");

//OutputPorts
uint64_t* beamer_3_ch16_buffer SECTION(".core8.data");
uint64_t* beamer_3_ch17_buffer SECTION(".core8.data");
volatile int* beamer_3_ch16_ready SECTION(".core8.data");
volatile int* beamer_3_ch16_valid SECTION(".core8.data");
volatile int* beamer_3_ch17_ready SECTION(".core8.data");
volatile int* beamer_3_ch17_valid SECTION(".core8.data");
...
int32_t beamer_3_ra_val_counter SECTION(".core8.data") = 0;
int32_t beamer_3_rb_val_counter SECTION(".core8.data") = 0;
...
int beamer_3_checkFlag(volatile int* flag){
	while(*flag == 0);
	return 1;
}
...
static inline void beamer_3_read_ra_val ( ) {
	int64_t ra_val;
	while(*beamer_3_ch4_valid == 0);//Blocking read
	ra_val = *beamer_3_ch4_buffer; //read input 
	*beamer_3_ch4_ready = 1; // channel ready to receive new data
	*beamer_3_ch4_valid = 0; // no valid input left on the channel
	
	beamer_3_complex_ra[beamer_3_ra_val_counter][1] = ra_val;
	beamer_3_ra_val_counter = (beamer_3_ra_val_counter + 1);

}
static inline void beamer_3_read_ra_val_done ( ) {
	beamer_3_ra_val_counter = 0;
...
static inline void beamer_3_read_rb_val ( ) {
	int64_t rb_val;
	while(*beamer_3_ch5_valid == 0);//Blocking read
	rb_val = *beamer_3_ch5_buffer; //read input 
	*beamer_3_ch5_ready = 1; // channel ready to receive new data
	*beamer_3_ch5_valid = 0; // no valid input left on the channel
	
	beamer_3_complex_rb[beamer_3_rb_val_counter][1] = rb_val;
	beamer_3_rb_val_counter = (beamer_3_rb_val_counter + 1);

}
static inline void beamer_3_read_rb_val_done ( ) {
	beamer_3_rb_val_counter = 0;
...
static inline void beamer_3_correlation ( ) {
	...
	while(*beamer_3_ch16_ready == 0);// Wait for the ready signal. 
	...
	*beamer_3_ch16_valid = 1; //(because) channel has new data 
	...
	while(*beamer_3_ch17_ready == 0);// Wait for the ready signal. 
	for(int beamer_3_correlation_i = 0; beamer_3_correlation_i < 9; beamer_3_correlation_i++){
...
int int_state_vars_beamer_3 SECTION(".core8.data") = 1;

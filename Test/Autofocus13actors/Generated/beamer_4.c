...
volatile int* beamer_4_ch6_ready SECTION(".core9.data");
volatile int* beamer_4_ch6_valid SECTION(".core9.data");
volatile int* beamer_4_ch7_ready SECTION(".core9.data");
volatile int* beamer_4_ch7_valid SECTION(".core9.data");

//OutputPorts
uint64_t* beamer_4_ch18_buffer SECTION(".core9.data");
uint64_t* beamer_4_ch19_buffer SECTION(".core9.data");
volatile int* beamer_4_ch18_ready SECTION(".core9.data");
volatile int* beamer_4_ch18_valid SECTION(".core9.data");
volatile int* beamer_4_ch19_ready SECTION(".core9.data");
volatile int* beamer_4_ch19_valid SECTION(".core9.data");
...
int32_t beamer_4_ra_val_counter SECTION(".core9.data") = 0;
int32_t beamer_4_rb_val_counter SECTION(".core9.data") = 0;
...
int beamer_4_checkFlag(volatile int* flag){
	while(*flag == 0);
	return 1;
}
...
static inline void beamer_4_read_ra_val ( ) {
	int64_t ra_val;
	while(*beamer_4_ch6_valid == 0);//Blocking read
	ra_val = *beamer_4_ch6_buffer; //read input 
	*beamer_4_ch6_ready = 1; // channel ready to receive new data
	*beamer_4_ch6_valid = 0; // no valid input left on the channel
	
	beamer_4_complex_ra[beamer_4_ra_val_counter][1] = ra_val;
	beamer_4_ra_val_counter = (beamer_4_ra_val_counter + 1);

}
static inline void beamer_4_read_ra_val_done ( ) {
	beamer_4_ra_val_counter = 0;
...
static inline void beamer_4_read_rb_val ( ) {
	int64_t rb_val;
	while(*beamer_4_ch7_valid == 0);//Blocking read
	rb_val = *beamer_4_ch7_buffer; //read input 
	*beamer_4_ch7_ready = 1; // channel ready to receive new data
	*beamer_4_ch7_valid = 0; // no valid input left on the channel
	
	beamer_4_complex_rb[beamer_4_rb_val_counter][1] = rb_val;
	beamer_4_rb_val_counter = (beamer_4_rb_val_counter + 1);

}
static inline void beamer_4_read_rb_val_done ( ) {
	beamer_4_rb_val_counter = 0;
...
static inline void beamer_4_correlation ( ) {
	...
	while(*beamer_4_ch18_ready == 0);// Wait for the ready signal. 
	...
	*beamer_4_ch18_valid = 1; //(because) channel has new data 
	...
	while(*beamer_4_ch19_ready == 0);// Wait for the ready signal. 
	for(int beamer_4_correlation_i = 0; beamer_4_correlation_i < 9; beamer_4_correlation_i++){
...
int int_state_vars_beamer_4 SECTION(".core9.data") = 1;

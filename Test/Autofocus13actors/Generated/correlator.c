...
volatile int* correlator_ch12_ready SECTION(".core12.data");
volatile int* correlator_ch12_valid SECTION(".core12.data");
volatile int* correlator_ch13_ready SECTION(".core12.data");
volatile int* correlator_ch13_valid SECTION(".core12.data");
volatile int* correlator_ch14_ready SECTION(".core12.data");
volatile int* correlator_ch14_valid SECTION(".core12.data");
volatile int* correlator_ch15_ready SECTION(".core12.data");
volatile int* correlator_ch15_valid SECTION(".core12.data");
volatile int* correlator_ch16_ready SECTION(".core12.data");
volatile int* correlator_ch16_valid SECTION(".core12.data");
volatile int* correlator_ch17_ready SECTION(".core12.data");
volatile int* correlator_ch17_valid SECTION(".core12.data");
volatile int* correlator_ch18_ready SECTION(".core12.data");
volatile int* correlator_ch18_valid SECTION(".core12.data");
volatile int* correlator_ch19_ready SECTION(".core12.data");
volatile int* correlator_ch19_valid SECTION(".core12.data");
volatile int* correlator_ch20_ready SECTION(".core12.data");
volatile int* correlator_ch20_valid SECTION(".core12.data");
volatile int* correlator_ch21_ready SECTION(".core12.data");
volatile int* correlator_ch21_valid SECTION(".core12.data");
volatile int* correlator_ch22_ready SECTION(".core12.data");
volatile int* correlator_ch22_valid SECTION(".core12.data");
volatile int* correlator_ch23_ready SECTION(".core12.data");
volatile int* correlator_ch23_valid SECTION(".core12.data");
...
int correlator_checkFlag(volatile int* flag){
	while(*flag == 0);
	return 1;
}
static inline void correlator_readInputs1 ( ) {
	...
	int64_t ba [9];
	int64_t bb [9];
	while(*correlator_ch12_valid == 0);//Blocking read
	...
	*correlator_ch12_valid = 0; // no valid input left on the channel
	...
	while(*correlator_ch13_valid == 0);//Blocking read
	for(int correlator_readInputs1_i = 0; correlator_readInputs1_i < 9; correlator_readInputs1_i++){
...
static inline void correlator_readInputs2 ( ) {
	...
	int64_t ba [9];
	int64_t bb [9];
	while(*correlator_ch14_valid == 0);//Blocking read
	...
	*correlator_ch14_valid = 0; // no valid input left on the channel
	...
	while(*correlator_ch15_valid == 0);//Blocking read
	for(int correlator_readInputs2_i = 0; correlator_readInputs2_i < 9; correlator_readInputs2_i++){
...
static inline void correlator_readInputs3 ( ) {
	...
	int64_t ba [9];
	int64_t bb [9];
	while(*correlator_ch16_valid == 0);//Blocking read
	...
	*correlator_ch16_valid = 0; // no valid input left on the channel
	...
	while(*correlator_ch17_valid == 0);//Blocking read
	for(int correlator_readInputs3_i = 0; correlator_readInputs3_i < 9; correlator_readInputs3_i++){
...
static inline void correlator_readInputs4 ( ) {
	...
	int64_t ba [9];
	int64_t bb [9];
	while(*correlator_ch18_valid == 0);//Blocking read
	...
	*correlator_ch18_valid = 0; // no valid input left on the channel
	...
	while(*correlator_ch19_valid == 0);//Blocking read
	for(int correlator_readInputs4_i = 0; correlator_readInputs4_i < 9; correlator_readInputs4_i++){
...
static inline void correlator_readInputs5 ( ) {
	...
	int64_t ba [9];
	int64_t bb [9];
	while(*correlator_ch20_valid == 0);//Blocking read
	...
	*correlator_ch20_valid = 0; // no valid input left on the channel
	...
	while(*correlator_ch21_valid == 0);//Blocking read
	for(int correlator_readInputs5_i = 0; correlator_readInputs5_i < 9; correlator_readInputs5_i++){
...
static inline void correlator_readInputs6 ( ) {
	...
	int64_t ba [9];
	int64_t bb [9];
	while(*correlator_ch22_valid == 0);//Blocking read
	...
	*correlator_ch22_valid = 0; // no valid input left on the channel
	...
	while(*correlator_ch23_valid == 0);//Blocking read
	for(int correlator_readInputs6_i = 0; correlator_readInputs6_i < 9; correlator_readInputs6_i++){
...
int int_state_vars_correlator SECTION(".core12.data") = 1;

...
volatile int* ranger_2_ch2_ready SECTION(".core1.data");
volatile int* ranger_2_ch2_valid SECTION(".core1.data");
volatile int* ranger_2_ch3_ready SECTION(".core1.data");
volatile int* ranger_2_ch3_valid SECTION(".core1.data");
...
int ranger_2_checkFlag(volatile int* flag){
	while(*flag == 0);
	return 1;
}
...
static inline void ranger_2_store_ra_results ( ) {
	...
	while(*ranger_2_ch2_ready == 0);// Wait for the ready signal. 
...
static inline void ranger_2_store_rb_results ( ) {
	...
	while(*ranger_2_ch3_ready == 0);// Wait for the ready signal. 
...
int int_state_vars_ranger_2 SECTION(".core1.data") = 1;

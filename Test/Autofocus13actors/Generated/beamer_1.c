...
volatile int* beamer_1_ch0_ready SECTION(".core6.data");
volatile int* beamer_1_ch0_valid SECTION(".core6.data");
volatile int* beamer_1_ch1_ready SECTION(".core6.data");
volatile int* beamer_1_ch1_valid SECTION(".core6.data");

//OutputPorts
uint64_t* beamer_1_ch12_buffer SECTION(".core6.data");
uint64_t* beamer_1_ch13_buffer SECTION(".core6.data");
volatile int* beamer_1_ch12_ready SECTION(".core6.data");
volatile int* beamer_1_ch12_valid SECTION(".core6.data");
volatile int* beamer_1_ch13_ready SECTION(".core6.data");
volatile int* beamer_1_ch13_valid SECTION(".core6.data");
...
int32_t beamer_1_ra_val_counter SECTION(".core6.data") = 0;
int32_t beamer_1_rb_val_counter SECTION(".core6.data") = 0;
...
int beamer_1_checkFlag(volatile int* flag){
	while(*flag == 0);
	return 1;
}
...
static inline void beamer_1_read_ra_val ( ) {
	int64_t ra_val;
	while(*beamer_1_ch0_valid == 0);//Blocking read
	ra_val = *beamer_1_ch0_buffer; //read input 
	*beamer_1_ch0_ready = 1; // channel ready to receive new data
	*beamer_1_ch0_valid = 0; // no valid input left on the channel
	
	beamer_1_complex_ra[beamer_1_ra_val_counter][1] = ra_val;
	beamer_1_ra_val_counter = (beamer_1_ra_val_counter + 1);

}
static inline void beamer_1_read_ra_val_done ( ) {
	beamer_1_ra_val_counter = 0;
...
static inline void beamer_1_read_rb_val ( ) {
	int64_t rb_val;
	while(*beamer_1_ch1_valid == 0);//Blocking read
	rb_val = *beamer_1_ch1_buffer; //read input 
	*beamer_1_ch1_ready = 1; // channel ready to receive new data
	*beamer_1_ch1_valid = 0; // no valid input left on the channel
	
	beamer_1_complex_rb[beamer_1_rb_val_counter][1] = rb_val;
	beamer_1_rb_val_counter = (beamer_1_rb_val_counter + 1);

}
static inline void beamer_1_read_rb_val_done ( ) {
	beamer_1_rb_val_counter = 0;
...
static inline void beamer_1_correlation ( ) {
	...
	while(*beamer_1_ch12_ready == 0);// Wait for the ready signal. 
	...
	*beamer_1_ch12_valid = 1; //(because) channel has new data 
	...
	while(*beamer_1_ch13_ready == 0);// Wait for the ready signal. 
	for(int beamer_1_correlation_i = 0; beamer_1_correlation_i < 9; beamer_1_correlation_i++){
...
int int_state_vars_beamer_1 SECTION(".core6.data") = 1;

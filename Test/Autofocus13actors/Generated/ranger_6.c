...
volatile int* ranger_6_ch10_ready SECTION(".core5.data");
volatile int* ranger_6_ch10_valid SECTION(".core5.data");
volatile int* ranger_6_ch11_ready SECTION(".core5.data");
volatile int* ranger_6_ch11_valid SECTION(".core5.data");
...
int ranger_6_checkFlag(volatile int* flag){
	while(*flag == 0);
	return 1;
}
...
static inline void ranger_6_store_ra_results ( ) {
	...
	while(*ranger_6_ch10_ready == 0);// Wait for the ready signal. 
...
static inline void ranger_6_store_rb_results ( ) {
	...
	while(*ranger_6_ch11_ready == 0);// Wait for the ready signal. 
...
int int_state_vars_ranger_6 SECTION(".core5.data") = 1;

...
volatile int* ranger_5_ch8_ready SECTION(".core4.data");
volatile int* ranger_5_ch8_valid SECTION(".core4.data");
volatile int* ranger_5_ch9_ready SECTION(".core4.data");
volatile int* ranger_5_ch9_valid SECTION(".core4.data");
...
int ranger_5_checkFlag(volatile int* flag){
	while(*flag == 0);
	return 1;
}
...
static inline void ranger_5_store_ra_results ( ) {
	...
	while(*ranger_5_ch8_ready == 0);// Wait for the ready signal. 
...
static inline void ranger_5_store_rb_results ( ) {
	...
	while(*ranger_5_ch9_ready == 0);// Wait for the ready signal. 
...
int int_state_vars_ranger_5 SECTION(".core4.data") = 1;
